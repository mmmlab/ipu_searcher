...
def simulateDynamicIdealSearcherHumFix(noise_type,current_block,p_threshold=0.99,targ_locs=None,fix_locs=None,locs=None):
    ...
    for current_trial in current_block.trials:
        for i in range(NR_TRIALS_SIMULATED):
            ...
            if(NOISE_TYPE=='notched'):
                trials.append(simulateDynamicIdealTrialHumFix(current_trial,(covariance_maps),p_threshold,dprime_unc_effect,sigma_ps_matrix));  
            if(NOISE_TYPE=='pink'):
                trials.append(simulateDynamicIdealTrialHumFixPink(current_trial,(covariance_maps_pink),(covariance_maps),p_threshold,dprime_unc_effect,sigma_ps_matrix));       
...
def simulateDynamicIdealTrialHumFix(current_trial,covariance_maps,p_threshold,dprime_unc_effect,sigma_ps_matrix):
